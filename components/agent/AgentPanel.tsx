  ...
  return (
    <div className="border-ai/20 bg-ai-soft/60 relative overflow-hidden rounded-md border px-2.5 py-1.5 text-xs">
      ...
      {state === "running" ? (
        <span
          aria-hidden
          className="animate-vetra-sweep pointer-events-none absolute inset-y-0 left-1/4 w-1/2 bg-gradient-to-r from-transparent via-white/60 to-transparent"
        />
      ) : null}
    </div>
  );
      ...
      >
        <div className="relative flex items-center justify-between gap-3 border-b px-4 py-3">
          ...
          {chatStatus.busy ? (
            <span
              aria-hidden
              className="animate-vetra-sweep via-ai absolute right-0 -bottom-px left-0 h-px bg-gradient-to-r from-transparent to-transparent"
            />
          ) : null}
        </div>
        {allowed ? (
